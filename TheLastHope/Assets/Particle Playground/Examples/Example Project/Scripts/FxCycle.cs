...
public class FxCycle : MonoBehaviour {
	...
	void CameraMovement () {
		...
		if (isSimulating) {
			...
			camSpline.timeOffset += (splineOffsetSpeed*initialSplineSpeed)*Time.deltaTime;

			// Camera movement along current spline
			camPivot.position = Vector3.Lerp (camPivot.position, camSpline.GetPoint(0f), camMovementSpeed*Time.deltaTime);

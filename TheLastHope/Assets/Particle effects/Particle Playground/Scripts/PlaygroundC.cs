...
namespace ParticlePlayground {
	...
	public class PlaygroundC : MonoBehaviour {
		...
		*************************************************************************************************************************************************/
		...
		public static float version = 2.21f;
		...
		*************************************************************************************************************************************************/
					...
					for (int s = 0; s<particleSystems.Count; s++) {
						for (int p = 0; p<particleSystems[s].particleCount; p++) {
							for (int m = 0; m<manipulators.Count; m++) {
								...
								if (particleSystems[s].IsReady() && particleSystems[s].IsAlive() && (manipulators[m].affects.value & 1<<particleSystems[s].GetLayer())!=0)
									PlaygroundParticlesC.CalculateManipulator(particleSystems[s], manipulators[m], p, globalDeltaTime, particleSystems[s].playgroundCache.life[p], particleSystems[s].playgroundCache.position[p], (particleSystems[s].IsLocalSpace()?manipulators[m].transform.localPosition:manipulators[m].transform.position)+manipulatorFix, particleSystems[s].IsLocalSpace());
								//}
							}
						}
					}
		...
		#if NETFX_CORE
		public static void RunAsync (IAsyncAction a) {
			lock (a) {
